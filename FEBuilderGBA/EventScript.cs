...
//                                 Debug.Assert(false);
        ...
        static string ReplaceHintWord(string hint)
        {
            ...
            {
                return R._("DISAで自軍ユニットを消す場合は、後でREVEALで再加入させてください。ユニットに離脱状態(Escape)フラグがセットされ、パーティから離れている状態になります。\r\n演出でユニットを消す場合は、CLEAを推奨します。CLEAは、すべてのユニットを消去しますが、離脱状態にはなりません。");
            }
