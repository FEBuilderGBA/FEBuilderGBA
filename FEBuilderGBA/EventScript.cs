...
//            }
        ...
        public static void ParseInfoString(string infostring, char symbol, out ArgType type, out String name)
        {
             ...
             Enum.TryParse(sp[1].ToUpper(), out type);
             if ( sp.Length >= 3)
             {
                name = sp[2];
             }
...
//                                 Debug.Assert(false);
        ...
        static string ReplaceHintWord(string hint)
        {
            ...
            {
                return R._("読込むユニットに、自軍の新規メンバーがいれば仲間にします。\r\nプレイヤー以外の敵やNPCに対して利用することもできます。");
            }
            else if (hint == "@LOAD2")
            {
                return R._("主にカットシーンと、敵とNPCユニットを読込むのに利用します。\r\nもしプレイヤーユニットの読込みに利用した場合、読込むユニットに、自軍の新規メンバーがいた場合は、章終了後に消滅するので注意してください。");
            }
            else if (hint == "@LOAD3")
            {
                return R._("生存ユニットしているユニットを、指定した座標に読み込みます。\r\n座標データ以外の情報は無視されます。\r\n主にプレイヤーユニットをたくさん表示するカットシーン用に使われます。");
            }
