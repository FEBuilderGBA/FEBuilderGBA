    ...
    public partial class SkillConfigSkillSystemForm : Form
    {
        ...
        static uint FindSkillPointer(string type, uint skip)
        {
            SkillSystemsTableSt[] table = new SkillSystemsTableSt[] { 
                ...
                new SkillSystemsTableSt{ name="ICON", skip =	0, data = new byte[]{0x38,0x00,0xFF,0xFF,0xFF,0xFF,0x80,0x22,0x0E,0x49,0x16,0x48,0x16,0x4B,0xFF,0xFF,0xFF,0xFF,0x2B,0x34,0x29,0x00,0x38,0x00,0x14,0x4B,0xFF,0xFF,0xFF,0xFF,0x26,0x70,0xFF,0xFF,0xFF,0xFF,0x00,0x47,0x01,0x33,0xCD,0xE7,0xC0,0x46}},
                new SkillSystemsTableSt{ name="ICON", skip =	0, data = new byte[]{0x38,0x00,0x00,0xF0,0xFB,0xF9,0x80,0x22,0x0E,0x49,0x16,0x48,0x16,0x4B,0x00,0xF0,0xF5,0xF9,0x2B,0x34,0x29,0x00,0x38,0x00,0x14,0x4B,0x00,0xF0,0xEF,0xF9,0x26,0x70,0xF8,0xBC,0x01,0xBC,0x00,0x47,0x01,0x33,0xCD,0xE7,0xC0,0x46}},
                ...
                new SkillSystemsTableSt{ name="TEXT", skip =	16, data = new byte[]{0x40,0x5D,0x08,0x49,0x40,0x00,0x40,0x18,0x00,0x88,0x00,0x28,0x00,0xD1,0x07,0x48,0x21,0x1C,0x4C,0x31}},
                ...
                new SkillSystemsTableSt{ name="ANIME", skip =	32, data = new byte[]{0x00,0x2B,0x00,0xD1,0x06,0x4B,0x38,0x1C,0x9E,0x46,0x00,0xF8,0x05,0x48,0x00,0x47}},
                new SkillSystemsTableSt{ name="ANIME", skip =	16, data = new byte[]{0x00,0xD1,0x33,0x1C,0x01,0x33,0x38,0x1C,0xFF,0xFF,0xFF,0xFF,0xF0,0xBC,0x11,0x48,0x00,0x47,0xF0,0xBC,0x10,0x48,0x00,0x47,0xF0,0xBC,0x10,0x48,0x00,0x47,0x18,0x47,0x6D,0xA1,0x05,0x08,0x35,0x8A,0x05,0x08,0x00,0x08,0x00,0x00,0x00,0x04,0x00,0x00,0x10,0xE3,0x06,0x08,0x8C,0xE5,0x06,0x08,0xC4,0xAE,0x02,0x08,0x55,0xA1,0x05,0x08}},
                new SkillSystemsTableSt{ name="ANIME", skip =	12, data = new byte[]{0x9D,0x2E,0x00,0x08,0x35,0x8A,0x05,0x08,0x00,0x08,0x00,0x00,0x00,0x04,0x00,0x00,0x10,0xE3,0x06,0x08,0x8C,0xE5,0x06,0x08,0xC4,0xAE,0x02,0x08,0x55,0xA1,0x05,0x08}},
            };
            ...
            foreach (SkillSystemsTableSt t in table)
            {
                ...
                bool[] mask;
                bool useMask = MakeMaskData(t.data, out mask);
                if (useMask)
                {
                    found = U.GrepPatternMatch(Program.ROM.Data, t.data, mask, start, end, 4);
                }
                else
                {
                    found = U.Grep(Program.ROM.Data, t.data, start, end, 4);
                }
        ...
        static bool MakeMaskData(byte[] grepdata, out bool[] out_mask)
        {
            ...
            {
                if (grepdata[i] == 0xFF && grepdata[i+1] == 0xFF && grepdata[i+2] == 0xFF && grepdata[i+3] == 0xFF)
                {
                    maskData.Add(true);
                    maskData.Add(true);
                    maskData.Add(true);
...
#if DEBUG
        public static void TEST_MakeMaskData()
        {
            byte[] data = new byte[] { 0x38, 0x00, 0x00, 0xF0, 0xFB, 0xF9, 0x80, 0x22, 0x0E, 0x49, 0x16, 0x48, 0x16, 0x4B, 0xFF, 0xFF, 0xFF, 0xFF, 0x2B, 0x34, 0x29, 0x00, 0x38, 0x00, 0x14, 0x4B, 0xFF, 0xFF, 0xFF, 0xFF, 0x26, 0x70, 0xF8, 0xBC, 0x01, 0xBC, 0x00, 0x47, 0x01, 0x33, 0xCD, 0xE7, 0xC0, 0x46 };
            bool[] mask = new bool[] { false,false,false,false,false,false,false,false,false,false,false,false,false,false,true, true, true, true, false,false,false,false,false,false,false,false,true, true, true, true,false,false,false,false,false,false,false,false,false,false,false,false,false,false };
            bool[] m;
            bool useMask = MakeMaskData(data, out m);
            Debug.Assert(useMask == true);

            for (int i = 0; i < mask.Length; i ++ )
            {
                Debug.Assert(m[i] == mask[i]);
            }
        }
#endif
