...
bl GetUnitsLeft @念のためもう一度数える. 0人であれば離脱完了
cmp r0 ,#0x00
ble Main_SetWinFlag
...
bl Escape @現在のユニットを離脱させる
                ...
                @ありえないと思うが、こいつが最後だったらまずいのでチェックする
bl GetUnitsLeft @残り人数の取得
cmp r0 ,#0x00
ble Main_SetWinFlag
b Main_Exit
...
GetUnitsLeft:
push {r4,r5,r6,lr}
ldr r5, =0x0202BE48	@gUnitArray	{J}
ldr r6, =0x0202BE48+(0x32*0x48)	@gUnitArray	{J}
@ldr r5, =0x0202BE4C	@gUnitArray	{U}
@ldr r6, =0x0202BE4C+(0x32*0x48)	@gUnitArray	{U}
mov  r4,#0x0
...
@7743's summonで召喚された亡霊戦士判定
ldrb r0, [r3, #0xF] @ramunit->status4
lsr  r0,#0x7
cmp r0,#0x01
beq IsPhantom_true
...
@ldr r0,=0x3004E50	@(操作キャラのワークメモリへのポインタ )	{U}
ldr r0,[r0]
bl IsPhantom
pop {r1}
bx r1
